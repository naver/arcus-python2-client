...
class arcus_node(object):
	...
	def do_arcus_command(self, command, timeout=0.2):
		...
		result = tn.read_until(message, timeout)
...
class zookeeper(object):
	...
	def __repr__(self):
		repr = '[ZooKeeper: %s] %s, %s' % (self.address, self.meta[0], self.meta[1])
	...
	def get_arcus_meta_all(self):
		if self.zk.exists('/arcus/meta') == None:
			self.zk.create('/arcus/meta', 'arcus meta info')
